...
*******************************************************************************/
	...
	static { 
		try {
			...
			alleleFrequency = new double[Parameters.generations][numGenes][];
			...
			for (int i = 0; i < numGenes; i++) { // Fill genome array with genes and their alleles
				...
				for(int z = 0; z < Parameters.generations; z++)
					alleleFrequency[z][i] = new double[numAlleles];
				
				for (int j = 0; j < numAlleles; j++) {
					for(int y = 0; y < Parameters.generations; y++)
						alleleFrequency[y][i][j] = 0;
